...
def getHeight(curr_node):
	if curr_node is None:
		return 0

	return 1 + max(getHeight(curr_node.left), getHeight(curr_node.right))

def findValue(v, root_node):
	if root_node is None:
		return False
	print root_node.value
	if root_node.value == v:
		return True

	return findValue (v,root_node.left) or findValue(v, root_node.right)
